...
| רעיונות | רעיונות לאפליקציות: כל אפליקציה מחזיקה רשימה מסודרת של רעיונות, פיצ'רים, פרומפטים שמורים (עם העתקה) וכותרות מקבצות. גרירה לסידור (עובד גם במגע), סימון בוצע מקפל למטה, וחיפוש חופשי בכל הרעיונות והפרומפטים |
| הדרכות | תיעוד מפגשי הדרכה: מקום, זמן, איש קשר, תשלום, קהל, תוכן, קישורים (עם העתקה), צ'קליסט פעולות משלימות וסימון תשלום. עובד גם אופליין — נשמר במכשיר ומסונכרן כשחוזרת הרשת. אפשר סנכרון דו-כיווני מלא ליומן גוגל (ר' `api/oauth_callback.php` ופעולות `gcal_*` ב-API) — כל שדה מקודד בתיאור האירוע, כך שעריכה ביומן (גם מהנייד) חוזרת לכאן |
